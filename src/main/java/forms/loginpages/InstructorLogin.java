...
import main.java.forms.panels.InstructorPanel;
...
public class InstructorLogin extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Instructor Login Page");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            PreparedStatement statement = Tools.connectToDB().
                    prepareStatement("SELECT COUNT(*) FROM instructor WHERE ID = ?");
            statement.setString(1, loginID);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
                    boolean exists = resultSet.getInt(1) > 0;
                    if (exists){
                        new InstructorPanel(loginID).setVisible(true);
                        this.setVisible(false);
                    }else{
                        JOptionPane.showMessageDialog(null, 
                                                                                "User ID does not exist in the instructor table. Please try again.",        
                                                                                "Error", JOptionPane.ERROR_MESSAGE);
                    }
            }
        } catch (SQLException ex) {
            System.out.println("Error in login button method of InstructorLogin class: "+ex.getMessage());
        }
